...
class Car:
    ...
    def rebound_action(self, game_map, point0, nr):
        ...
        for vi in range(5, 370, 10):
            ...
            x2 = x0 + radius * math.cos(angle + math.radians(7))
            y2 = y0 + radius * math.sin(angle + math.radians(7))

            point1 = [int(x1), int(y1)]

            if game_map.get_at([int(x1), int(y1)]) == BORDER_COLOR and \
                    game_map.get_at([int(x2), int(y2)]) == OutBORDER_COLOR:
                point1 = [int(x1), int(y1)]
        ...
        self.carangle = (self.carangle % 360 + 360) % 360
        self.position[0] -= math.cos(math.radians(360 - self.carangle)) * 10 * np.sin(np.radians(angle_theta)) * self.speed
        self.position[1] -= math.sin(math.radians(360 - self.carangle)) * 10 * np.sin(np.radians(angle_theta)) * self.speed
