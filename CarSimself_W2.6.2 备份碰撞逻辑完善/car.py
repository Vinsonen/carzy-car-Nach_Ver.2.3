...
class Car:
    ...
    def rebound_action(self, game_map, point0, nr):
        ...
        self.position[0] -= math.cos(math.radians(360 - self.carangle)) * 8 * np.sin(np.radians(angle_theta)) * self.speed
        self.position[1] -= math.sin(math.radians(360 - self.carangle)) * 8 * np.sin(np.radians(angle_theta)) * self.speed
        # change angle
        if nr % 2 == 1:
            kt = -1
        else:
