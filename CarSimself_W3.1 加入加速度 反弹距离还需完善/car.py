...
class Car:
    ...
    def __init__(self, position, carangle, power, speed_set, radars, bit_volt_wert_list, distance, time):
        ...
        self.swert = 0
        self.speed = 0
        self.sollspeed = self.sollspeed(power)
    ...
    def rebound_action(self, game_map, point0, nr):
        # sfa反射角度的计算
        # self.speed = 1
        #pygame.draw.circle(game_map, (100, 0, 255), point0, 1)  # hua碰撞点
        if nr in [1, 2]:
            x0 = point0[0]
            y0 = point0[1]
            # point1 = [0, y0]
            ...
            k0 = -1
            self.position[0] += k0 * math.cos(math.radians(360 - self.carangle)) * 8 * self.speed
            self.position[1] += k0 * math.sin(math.radians(360 - self.carangle)) * 8  * self.speed
# * np.sin(np.radians(angle_theta))
            # change angle
            if nr == 1:  # left top
                kt = -1
            else:  # right top
            ...
            turn_angle = 10 * np.cos(np.radians(angle_theta))  # 设置转角为10度乘上系数
    ...
    def Lenkeinschlagsänderung(self):
        ...
        car_radius = int(CAR_Radstand / math.tan(angle_rad) + CAR_Spurweite / 2)  # 轮距CAR_Radstand
    ...
    def Geschwindigkeit(self, power):
        ...
        if power == 0:
            ...
            if (speed + beschleunigung * 0.01) <= maxspeed:
