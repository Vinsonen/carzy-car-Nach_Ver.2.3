...
class Car:
    ...
    def __init__(self, position, carangle, fwert, speed_set, radars, bit_volt_wert_list, distance, time):
        ...
        self.swert = 0
        self.speed = 0
        self.sollspeed = self.sollspeed(fwert)
    ...
    def rebound_action(self, game_map, point0, nr):
        ...
        if nr in [1, 2]:
            ...
            turn_angle = 7 * np.sin(np.radians(angle_theta))     #   np.sin(np.radians(2*(360-angle_theta)))+0.1
    ...
    def getmotorleistung(self, fwert):
        if 18 > fwert > -18:
            ...
            return min(fwert, 100)
        else:
            return max(fwert, (-1) * 100)
