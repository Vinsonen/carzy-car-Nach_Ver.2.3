...
def run_simulation(genomes, config):
    ...
    map_text = "Blackboard.png"
    nets = []
    cars = []
    positionMaxX = [0, 0]
    positionMinX = [1920, 1080]
    ...
    for i, g in genomes:
        ...
        new_car = Car([1190 * f, 600 * f], 0, 18, False, [], [], 0, 0)
        cars.append(new_car)
    ...
    while True:
        ...
        for i, car in enumerate(cars):
            if car.position[0] > positionMaxX[0]:
                positionMaxX[0] = car.position[0]
                positionMaxX[1] = car.position[1]

            if car.position[0] < positionMinX[0]:
                positionMinX[0] = car.position[0]
                positionMinX[1] = car.position[1]
            dreh_radius_sim =(positionMaxX[0]-positionMinX[0])/2
            dreh_radius_real = sim_to_real(dreh_radius_sim)



            font.render_to(screen, (100, 100), f"radAngle_real:{car.radangle} maxXPosition : ({positionMaxX[0]:.1f},  {positionMaxX[1]:.1f})  "
                                       f"minXPosition: ( {positionMinX[0]:.1f},  {positionMinX[1]:.1f} )"
                                       f"  radius: {dreh_radius_sim:.2f} px  {dreh_radius_real:.2f} cm", (255, 0, 100))
