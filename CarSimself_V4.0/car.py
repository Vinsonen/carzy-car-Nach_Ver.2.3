...
class Car:
    ...
    def rebound_action(self, game_map, point0, nr):
        ...
        if nr in [1, 2]:
            ...
            turn_angle = 7 * np.sin(np.radians(2*angle_theta))     #   np.sin(np.radians(2*(360-angle_theta)))+0.1
    ...
    def check_collision(self, game_map):
        ...
        for point in self.corners:
            ...
            # Assumes Rectangle
            nr = nr + 1
            # a = game_map.get_at((840, 933))  # (237, 28, 36)
            # if game_map.get_at((int(point[0]), int(point[1]))) == a : #(255, 0, 0):
