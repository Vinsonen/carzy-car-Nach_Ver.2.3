...
# 加载库文件
lib = ffi.dlopen("C:/Users/Vinson/Desktop/毕业论文/CarSimself_W2.5 - 修改碰撞逻辑更多/IF/x64/Debug/IF.dll")
f = car.f
...
def run_simulation(genomes, config):
    ...
    while True:
        ...
        text_rect = text.get_rect(center=(900 / 15 * f, 450 / 15 * f))

        screen.blit(text, text_rect)
        text = alive_font.render("Still Alive: " + str(still_alive), True, (0, 0, 0))
        text_rect = text.get_rect(center=(900 / 15 * f+10, 490 / 15 * f+10))
        screen.blit(text, text_rect)
...
def regelung(cars):
    ...
    for i, car in enumerate(cars):
        ...
        if car.angle_enable:
            ...
            car.radangle=car.getwinkel(car.swert)
...
def moment_recover(file_text_date):
    ...
    for data in deserialized_data:
        recover_cars.append(
            Car(data['position'], data['carangle'], data['speed'], data['speed_set'], data['radars'],
                data['analog_wert_list'], data['distance'], data['time']))
